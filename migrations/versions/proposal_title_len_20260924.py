"""trainer_course_proposals.title: varchar(50) -> varchar(120).

Назви доповідей у 50 символів не вміщались ("Багата тромбоцитами плазма
PRP та інші аутологічні ..."); куратор попросила щонайменше 100, узгоджено
120. Межа форми, підказка й maxlength беруться з
TrainerCourseProposal.TITLE_MAX, тож тут змінюється лише колонка.
...
Revision ID: proposal_title_len_20260924
Revises: lect_cert_downloaded_20260923
"""
import sqlalchemy as sa
from alembic import op

revision = 'proposal_title_len_20260924'
...
def upgrade():
    with op.batch_alter_table('trainer_course_proposals', schema=None) as batch_op:
        batch_op.alter_column('title', existing_type=sa.String(50),
                              type_=sa.String(120), existing_nullable=False)
...
def downgrade():
    ...
    with op.batch_alter_table('trainer_course_proposals', schema=None) as batch_op:
        batch_op.alter_column('title', existing_type=sa.String(120),
