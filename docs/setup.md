...
## Заливка контенту курсу плазмогелю

Тексти продажної сторінки курсу плазмогелю (цифри довіри, картки результату,
«для кого», програма, плашка практики, FAQ, підписи галереї, тарифи, цифри
тренера, відгуки) заливаються командою, а не міграцією: це контент, і редакція
має могти переписати кожне поле в адмінці.

```bash
# Спершу подивитися, що буде змінено (транзакцію буде відкочено)
...
Команда ідемпотентна: порожнє поле заповнює, заповнене лишає (щоб не з'їсти
редакторську правку), повторний запуск не плодить блоків програми, тарифів чи
відгуків. `--force` перезаписує вже заповнені поля текстами еталона, `--slug`
вказує інший курс. Відгуки створюються **чернетками**: в еталоні це збірні
формулювання без автора й без оцінки, тож публікує їх людина в `/admin/reviews`.

Фото галереї команда не вантажить -- лише підписує вже прив'язані до курсу
(за порядком). Тарифи чіпляються до найближчого майбутнього опублікованого
проведення; ціну наявного тарифу без `--force` не рухає.
