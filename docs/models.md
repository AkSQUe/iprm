...
завданні («не вводити спеціальності вдруге в розділі "Контент"») їх не було.

- **Перелік у блоці «Чи це ваш курс» більше не набирається руками** — він
  збирається з поля «Спеціальності (для сертифіката)». Старий текст із
  заходів ніхто не чистив: доки редактор не прибере його з «Додаткового
  опису», перелік видно двічі.
- **Блок показується і тоді, коли додаткового опису немає взагалі** —
  досить обраних спеціальностей. І навпаки: разом із порожнім описом
  зникає підзаголовок «Вам варто приєднатися, якщо ви», а не лишається
  висіти над порожнім списком.
- **Порядок назв — номенклатурний** (розділи I–IV, далі порядок усередині
  розділу), а не той, у якому їх обрали у формі. Два заходи з тим самим
  набором спеціальностей читаються однаково.
- **Спеціальність, деактивована в довіднику, далі показується там, де вже
  проставлена.** Зникає вона лише з вибору для нових заходів — інакше
  актуалізація номенклатури мовчки міняла б зміст сторінок архівних курсів.
- **Той самий код двічі дає один пункт** — і на сторінці, і в рядку
  «Спеціальності:» сертифіката. Кліком у формі дубль не зробити, але
  зібраний руками POST чи ручний UPDATE його кладуть.
- **У формі заходу перелік видно ще до збереження:** прев'ю над полем
  «Додатковий опис» перемальовується одразу після вибору спеціальності.
  Після невдалої валідації воно показує подане, а не збережене, — інакше
  в одній формі висіли б два різні переліки.
- **Блок бере спеціальності КУРСУ.** Спеціальності, задані на рівні
  проведення, лишаються тим, чим були, — перевизначенням для сертифіката
  конкретного проведення; на сторінку курсу вони не впливають.
- **Онлайн-курси спеціальностей не мають** — там блок працює як раніше,
  лише на ручних рядках: онлайн-курс не є заходом БПР.
- **На `/ru/` і `/en/` назви беруться з перекладів довідника**, а не з
  поля курсу; спеціальність без перекладу показується українською.
  Переклад «Додаткового опису» лишається у мовних вкладках самого поля.
- **Якщо довідник недоступний, сторінка малюється без переліку**, а не
  падає: заходу без ручного опису блок просто не покажуть.
- **Колонка xlsx називається «Цільова аудиторія (допис)»**; файли зі
  старим підписом імпортуються далі. Самих спеціальностей у вигрузці
  немає — через xlsx їх не задати, тільки в адмінці.
- **Партнерське API віддає спеціальності окремим ключем `bpr_specialties`**
  (завжди українською). `target_audience` лишився на місці й несе тепер
  лише ручний допис.
...
Рішення, ухвалені при написанні плану та в ході реалізації; у первісному
завданні («замінити п'ять типів на дванадцять») їх не було.

- **Вид заходу можна перевизначити на рівні проведення.** Порожній вибір
  у формі проведення означає «як у курсу» і лягає в БД як NULL. Один курс
  можна раз провести тренінгом, раз фаховою школою.
- **Розклад показує вид заходу проведення, сторінки курсу — вид курсу.**
  І списковий, і календарний вигляд узгоджені між собою.
...
- **Тип, на який посилається хоч один курс, видалити не можна** — його
  знімають з активних. Перемапування курсів не передбачено.
- **Сертифікат друкує знахідний і родовий відмінки з довідника** і бере
  ефективний вид заходу проведення. Доти документ лише переводив називний
  у нижній регістр («завершив конференція»).
- **xlsx-імпорт приймає і код (`seminar`), і українську назву
  («Семінар»)**, без чутливості до регістру й крайніх пробілів; застарілі
  коди проходять. Невідоме значення зупиняє рядок із помилкою, що називає
  саме значення.
- **Порожня комірка типу при імпорті дає `seminar`** (раніше `course` —
  його деактивовано).
- **Експорт пише канонічну українську назву** незалежно від мови, на якій
  адміністратор працює в панелі, інакше файл не заходив би назад.
- **Drop-down у шаблоні курсів пропонує лише активні типи** і має близько
  34 символів запасу за межею Excel: ще приблизно два нові види заходів,
  після чого список у файлі зникне.
- **Партнерське API віддає ефективний код проведення**, як і решта полів
  того самого словника.
- **Права `event_types.*` роздаються вручну** в `/admin/access`; жодній
  ролі, крім `super_admin`, вони не видаються автоматично.
- **Код нового виду заходу** — лише малі латинські літери, цифри, `_` і
  `-`, до 30 символів: він їде в партнерське API та xlsx.
...
- **Порядок тренерів у полі — це роль, а не оформлення.** Перший у списку
  вважається головним лектором: його ПІБ і підпис ідуть на сертифікат
  учасника, його ім'я показує картка заходу в каталозі. Переставили порядок
  — змінили, хто підписує.
- **Поле «Інформація про спікера» більше не існує.** Раніше туди руками
  переписували біографію тренера, і цей текст ніде на сайті не виводився.
  Тепер блок збирається з карток, і переписувати нічого не треба.
- **Заповненою вважається картка, де є біографія, роль і фото** — рівно те,
  що рендерить публічний блок. Попередження про неповну картку адресне
  («немає фото»), але зберегти захід не заважає: худа картка — робочий стан,
  а форма, що відмовляла б, лише підштовхувала б прибрати тренера зі списку.
- **У проведення порожній список означає «взяти тренерів курсу».** Форма
  показує лише ВЛАСНИЙ список проведення і ніколи не підставляє успадкований
  — інакше перше ж збереження перетворило б успадкування на застиглу копію,
  і далі курс із проведенням розходились би непомітно.
- **Список проведення перекриває курсовий повністю, а не доповнює.** Вказали
  у проведення хоч одного тренера — курсові більше не враховуються ніде: ні
  на сторінці, ні у фільтрі реєстрацій, ні в розсилці.
- **Порядок задається кнопками на чіпах**; без JavaScript лишається звичайний
  мультиселект, і порядок у ньому алфавітний. Тобто при вимкненому JS
  головного лектора обирає прізвище.
- **Деактивований тренер лишається у формі заходу, якщо вже до нього
  прив'язаний.** Інакше збереження будь-якої дрібниці — навіть заголовка —
  тихо викидало б його зі складу разом із сертифікатом і сповіщеннями.
- **Лекторський сертифікат видається кожному тренеру окремо**, зі своїм
  номером і повними балами: бали не діляться, бо захід провів кожен із них.
  Кнопки «видати всім» немає — кожна видача лишається свідомою дією.
- **Правило сповіщень «Тренер заходу» шле всім тренерам.** Тренер, який
  водночас менеджер, отримує один лист, а не два; тренер без заповненої
  пошти пропускається мовчки.
- **У xlsx колонка «Тренер» стала «Тренери» — старі вигрузки не
  імпортуються.** Файл без цієї колонки відхиляється з «бракує колонок»:
  мовчазна альтернатива стерла б тренерів усім курсам у файлі. Перед
  імпортом треба вигрузити наново.
- **Роздільник у клітинці — крапка з комою, і вона має пріоритет** (кома
  приймається лише в клітинці без жодної `;`, бо кома буває всередині ПІБ:
  «Іванов І. І., PhD»). Випадний список прибрано — Excel не перевіряє
  клітинку з кількома значеннями й відхиляв би правильне введення; аркуш
  «Тренери» лишився, з нього копіюють точні написання. Тезки вимагають slug:
  імпорт не вгадує, кого з двох мали на увазі.
- **Партнерське API віддає масив `trainers` замість `trainer`**, і в ньому
  з'явився `bio`. Поле `email` лишилось навмисно: MM Medic читає його з
  живої відповіді, щоб зіставити тренера з акаунтом. Онлайн-курси
  множинності не отримали — там як був один тренер, так і лишився.
