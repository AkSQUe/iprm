...
Рішення, ухвалені при написанні плану та в ході реалізації; у первісному
завданні («після повного опису курсу показувати актуальні проведення») їх
не було.

- **Проведення з набраною групою зникає з переліку цілком**, а не сіріє
  бейджем «Реєстрацію закрито». Стан «закрито» лишився на `/courses`: там
  розклад показує всі заходи, а не лише ті, на які можна записатись.
- **Порожній блок дат розрізняє «курс не планують» і «на найближчу дату не
  встигли»** і в другому випадку називає саме цю дату («Групу на 12.10.2026
  вже набрано»). Інакше сторінка казала б, що проведень немає, хоча вони
  заплановані.
- **Коли є і відкриті, і набрані дати, під переліком лишається рядок «ще N
  групу вже набрано».** Без нього перелік із трьох дат читається як увесь
  розклад курсу.
- **Від шести відкритих дат перелік розкладається за містами** (поріг --
  `GROUPING_THRESHOLD` у `services/course_listing.py`). Наскрізної
  хронології між групами тоді немає: групи йдуть за найранішою датою
  всередині, а дати всередині групи -- хронологічно.
- **Проведення без вказаного місця й не онлайн потрапляє в групу «Місце
  уточнюється».** Група існує лише в режимі групування -- у плоскому
  переліку такі дати нічим не виділяються.
- **Ціна в hero і в липкій панелі рахується лише по відкритих датах**, а
  без них береться базова ціна курсу. Інакше сторінка обіцяла б ціну
  набраної групи, дати якої в переліку вже немає.
- **Лічильник місць у шапці показує залишок саме найближчої відкритої
  дати** і зникає, коли в неї не задано місткість. Раніше він брав перше
  проведення, де місткість узагалі є, тож міг приписати найближчій даті
  залишок пізнішої.
- **Блок «Оберіть формат участі» зникає разом із тарифами, коли найближче
  проведення з тарифами набране.** Стан «Продаж завершено» в картках
  тарифів став недосяжним і прибраний.
- **Кнопка «Повідомити про нову дату» дописує в заявку перший рядок «Чекає
  на нову дату проведення.»** Позначка ставиться на будь-якій заявці зі
  сторінки без відкритих дат -- зокрема з курсу, який жодного разу не
  планували.
- **Цей рядок не перекладається:** заявка з `/ru/` чи `/en/` приїде з ним
  українською. Його читає адміністратор у своїй мові роботи, а не
  відвідувач.
- **У розмітці для Google дат більше, ніж бачить відвідувач:**
  `hasCourseInstance` перелічує всі заплановані проведення, зокрема
  набрані -- заповненість групи не робить подію незапланованою. Натомість
  `Offer.availability` при жодній відкритій даті стає `SoldOut`, і сам
  вузол `Offer` є лише тоді, коли в курсу задана базова ціна.
