...
| `topic` | String(255) | Необов'язкова тема цього проведення; null = взяти `Course.title` (`effective_title` / `effective_title_for(lang)`). Перекладна (`translations`) |
...
| `event_type` | String(30) | Необов'язковий override коду з довідника `EventType`; null = взяти з `Course.event_type` (`effective_event_type`). Картка дати на сторінці курсу показує лише `distinct_event_type` -- вид, що відрізняється від курсового; рядок «Графіка курсів» показує `effective_event_type` завжди, бо там картки курсу поруч немає |
...
### Особливості в користуванні (тема проведення)

Рішення, ухвалені при написанні плану та в ході реалізації; у первісному
завданні («додати необов'язкове поле "Тема" під полем "Курс"») їх не було.

- **Плейсхолдер поля показує назву курсу** -- саме те, що стане назвою
  заходу, якщо тему лишити порожньою. Порожнє поле інакше читається як
  «назви не буде».
- **Тема з самих пробілів дорівнює незаповненій.** У БД лягає NULL, а не
  порожній рядок, тож захід вертається до назви курсу, а не залишається
  без імені.
- **У картці проведення поруч із темою лишається підпис «курс: ...».**
  Щойно тема підмінила заголовок картки, з неї стало не видно, до якого
  курсу належить дата.
- **Каталог `/courses` і сторінка курсу звуть саму дату темою, а картку
  курсу -- ні.** Картка представляє курс, а не проведення, тож там завжди
  стоїть назва курсу, навіть коли всі його дати мають власні теми.
- **Випадайка прив'язки заходу в Meta Lead Ads теж показує тему.**
  Менеджер обирає дату за тим самим підписом, який побачить у листі й у
  документі.
- **Тема перекладається окремо від курсу** (`/admin/translations`,
  entity `course_instance`), а кнопка «Переклади» з'являється на картці
  лише тоді, коли тема заповнена: без теми перекладати нічого, і назву
  курсу перекладають у картці курсу.
- **Проведення без теми не потрапляє в xlsx перекладів.** Аркуш
  «Проведення» у розділі «Переклади розкладу» містить тільки дати з
  власною темою -- решта рядків була б порожньою роботою для перекладача.
- **У xlsx-розкладі теми немає.** Масове редагування розкладу її не бачить
  і не змінює -- як і виду заходу, міста та спеціальностей; тема
  заводиться лише в картці проведення.
- **Сертифікат бере тему українською незалежно від того, з якої мовної
  версії сайту його видали.** Виданий документ не має залежати від
  локалі того, хто натиснув кнопку.
- **Тема, додана після видачі сертифіката, його не переписує.** Назва
  заходу зберігається знімком у момент видачі; підхопити нову тему може
  лише відкликання й повторна видача.
- **Партнерське API і вихідний вебхук у mm-medic віддають тему в полі
  назви заходу.** Партнерський сайт побачить тему, а не назву курсу, --
  зокрема у вітрині заходів і в резервуванні витратних матеріалів.

### Особливості в користуванні (бали БПР)

Рішення, ухвалені при написанні плану та в ході реалізації; у первісному
завданні їх не було.

- **На сторінці курсу видно обидва поля балів, на сторінці проведення --
  лише ті, що має його формат.** Курс -- шаблон без формату, тож обидва
  default-и потрібні; проведення показує онлайнове поле для онлайну,
  офлайнове для очного, обидва для гібрида.
- **Перемикання формату не стирає бали іншого формату:** приховане поле
  ховається атрибутом `hidden`, а не вимикається, тому далі сабмітить своє
  значення. Захід нерідко стає гібридним уже після створення, і введене
  раніше число мусить пережити цей перехід.
- **Кома і крапка приймаються однаково, а показується завжди кома без
  хвостових нулів.** Поле текстове, а не числове: `<input type="number">`
  не приймає кому, і «4,5» доїхало б до сервера порожнім. `9,00` у полі не
  побачите -- там буде `9`. У PDF сертифіката кома стоїть завжди, незалежно
  від мови інтерфейсу.
- **Дробові бали відмінюються окремою формою: «4,5 бала БПР», а не
  «4,5 балів».** Слов'янське правило працює із залишками цілого і для
  дробових не годиться, тож у наборі форм з'явилась четверта.
- **Гібридний захід у вузьких місцях показує діапазон, у широких -- обидва
  значення з підписами.** Тег розкладу й картка курсу друкують `7,5-9`,
  сторінка заходу й форма реєстрації -- «Онлайн 7,5 · Офлайн 9».
- **Формат участі береться з тарифу, але перекривається руками в картці
  учасника** -- порожній варіант називається «За тарифом». Гібридний захід
  без тарифу трактується як очна участь: зайве попередження нешкідливе, а
  пропущене -- ні.
- **Онлайнового учасника не пустять у тест, якщо заповнені лише офлайнові
  бали.** Він побачить «дані БПР не заповнені» замість сертифіката з
  порожнім місцем під бали.
- **Порожнє поле балів при підтвердженні присутності означає «нарахувати
  належне за форматом», а не нуль.** Нечислове значення дає повідомлення,
  а не помилку сервера.
- **У xlsx замість однієї колонки балів тепер дві, плюс колонка формату
  участі.** Імпорт приймає «7,5» так само, як «7.5» -- саме це приходить з
  Excel з українською локаллю. Експорт віддає СИРЕ значення формату:
  порожньо означає «за тарифом», тож круговорот «вивантажив -> виправив ->
  завантажив» не заморожує похідне значення в кожному рядку.
- **Партнерське API замінило ключ `cpd_points` парою `cpd_points_online` /
  `cpd_points_offline` і підняло `api_version` до 1.1.** `null` там означає
  «цей формат участі захід не пропонує», а не «нуль балів»: суто очний
  захід віддає `cpd_points_online: null`, навіть якщо default курсу
  заповнений.
- **Розетки з кількістю балів на сертифікаті немає, і це не пропуск.**
  `points_badge_url` роками обчислював шлях до неї й клав у контекст
  шаблону, але жоден шаблон його не рендерив -- ані звичайний сертифікат,
  ані компактний. 10.09.2026 цю мертву проводку прибрано разом із єдиним
  файлом `10-points-BPR.webp`. Якщо розетка колись знадобиться, її треба
  робити наново і одразу підключати до розмітки.
- **Міграція поклала ОФЛАЙНОВЕ значення в обидві колонки.** На гібридних
  заходах, де бали різняться, онлайнові треба проставити руками -- інакше
  розділення технічно є, а фактично ні. Сама міграція ручною не є: деплой
  виконує `flask db upgrade` сам.
...
| `name_genitive` | String(120) | Родовий з малої — для тексту сертифіката лектора («…тренера наукової конференції») |
...
xlsx-вигрузка проведень (`export_instances_xlsx`) має колонку «Вид
заходу» з випадайкою, і вона ЄДИНА в розкладі дозволяє порожнє значення
(`allow_blank=True`): порожньо тут означає «як у курсу». Тому експорт
друкує лише ВЛАСНИЙ вид дати, а не `effective_event_type` — інакше
перше ж «вивантажив і завантажив назад» записало б курсовий тип у кожну
дату твердою копією, і наступна зміна виду курсу вже нікуди не дійшла б.
Колонка опційна (`OPTIONAL_INSTANCE_COLS`): файл, вивантажений до її
появи, лишає збережені override як є, а не занулює їх.
...
- **Порожній варіант називає успадкований вид у дужках** — «– Як у курсу
  (Тренінг) –» — скрізь, де курс уже відомий: у правці дати й у створенні
  з картки курсу (`?course_id=`). На чистому `/instances/new` курс обирають
  у тій самій формі, тож підпис лишається голим. Курс для всіх успадкованих
  полів проведення розвʼязує один `_known_course` у `routes_instances.py`
  (ним же живиться підпис рівня складності) — третьої копії цих пʼяти
  рядків заводити не треба.
- **У реєстрі проведень вид стоїть другим поверхом під назвою курсу, а не
  окремою колонкою.** Восьма колонка повернула б горизонтальну прокрутку,
  заради усунення якої й заведено `page-admin-instances.css` (таблиця
  впирається в 1050px при 1078 доступних на 1440). Друкується ФАКТИЧНИЙ
  вид дати; перевизначений позначено зірочкою, бо оком треба помітити саме
...
- **Картка дати на сторінці курсу називає вид, лише коли він
  відрізняється від курсового** (`distinct_event_type`). Доти вона не
  називала його зовсім, і дата, перевизначена на тренінг під
  курсом-семінаром, публічно виглядала семінаром: hero-чип угорі друкує
  вид КУРСУ. Правило те саме, що в сусіднього рівня складності.
  У «Графіку курсів» на `/courses` правило ІНШЕ й свідомо: там вид
  ефективний і стоїть завжди, бо графік -- окрема секція нижче каталогу,
  картки курсу поруч із рядком немає, і вид у рядку єдиний, хто називає
  захід.
- **Реєстр проведень фільтрується за ЕФЕКТИВНИМ видом.** Простий
  `CourseInstance.event_type == code` (як у курсів) згубив би всі дати,
  що вид успадковують, тобто майже всі, — і екран виглядав би
  правдоподібно порожнім, а не зламаним. Успадкування перевіряється
  через EXISTS, а не join: `_instances_query` приєднує Course лише під
  пошук.
- **Звіт реєстру (`/admin/instances/report.xlsx`) друкує ефективний
  вид**, як і решта його колонок (`effective_title`, `effective_price`).
  Це звіт, а не файл для зворотного завантаження.
...
- **Обидва аркуші звуть колонку «Вид заходу».** Аркуш курсів звався
  «Тип», хоча брав коди з того самого довідника; старий підпис лишився
  аліасом у `LEGACY_HEADERS`, бо вигрузки з ним у менеджерів на руках.
...
- **Порожня комірка типу при імпорті дає `seminar`** (раніше `course` —
  його деактивовано). Це про аркуш КУРСІВ; у розкладі порожня комірка
  означає «як у курсу» і знімає override у NULL.
...
- **Drop-down пропонує лише активні типи** і має близько 34 символів
  запасу за межею Excel: ще приблизно два нові види заходів, після чого
  список у файлі зникне. Стосується ОБОХ аркушів -- курсів і розкладу.
...
## Specialty (довідник спеціальностей)

Номенклатура спеціальностей для рядка «Спеціальності:» на сертифікаті —
редагована таблиця, а не константа в коді: перелік мусить збігатися з
чинною номенклатурою МОЗ і мінятись без деплою. Сторінка адмінки —
`/admin/specialties` (права `specialties.view|manage|delete`).

Джерело наповнення — Додаток 1 до Порядку проведення атестації
працівників сфери охорони здоров'я (`docs/dn_650_16042025_dod_3.pdf`).
`tools/parse_specialties.py` розбирає його один раз у
`app/data/specialties.py`; далі цей файл читає лише міграція
`specialties_20260909`, а довідник живе в БД. У рантаймі PDF не читається,
`pypdf` у залежності проєкту не входить.

| Поле | Тип | Опис |
|------|-----|------|
| `id` | BigInt | PK |
| `code` | String(60) unique | Транслітерований слаг української назви, згенерований один раз і заморожений; саме його зберігають `Course.bpr_specialty_codes` і `CourseInstance.bpr_specialty_codes` |
| `name` | String(200) | Назва з номенклатури як є; перекладна на ru/en через `TranslatableMixin` |
| `section` | String(20) | Розділ Додатка: `medical`, `pharmacy`, `professionals`, `specialists` — шапка групи у виборі |
| `is_group` | Boolean | Службове узагальнення («усі лікарські спеціальності») — рядок довідника, а не окремий стан поля |
| `sort_order` | Integer | Порядок усередині розділу; узагальнення мають `0` і стоять першими |
| `is_active` | Boolean | Чи пропонується у виборі нового курсу/проведення |

Курс і проведення посилаються на довідник списком КОДІВ у JSON-колонці, а
не зовнішнім ключем: код стабільний, читається в дампі й переживає
перезаливку довідника. Цілісність тримає заборона видаляти рядок, який
десь уже вжито.

Служба `app/services/specialties.py` кешує довідник на час запиту (`g`) і
дає `names()`, `line()`, `choices(current=None)`, `usage()`,
`clean_codes()`.

### Особливості в користуванні

Рішення, ухвалені при написанні плану та в ході реалізації; у первісному
завданні («перетворити поле на множинний вибір із довідника») їх не було.

- **У довідник заведено лише частину 1 Додатка — 192 позиції плюс чотири
  узагальнення.** Блок «Профілі роботи за спеціальностями», а це приблизно
  третина документа, свідомо не заводився: це не спеціальності, і у виборі
  для сертифіката він плутав би.
- **«Усі лікарські спеціальності» — звичайний рядок довідника**, а не
  окремий стан поля: обирається як будь-яка позиція, стоїть першою у своєму
  розділі й друкується власним текстом з малої літери, бо в сертифікаті йде
  одразу після «Спеціальності: ».
- **Без JavaScript поле лишається робочим:** видно нативний
  `<select multiple>` із тим самим списком. Чіпи, пошук і клавіатура —
  покращення поверх нього, сабміт іде тим самим ім'ям поля.
- **Пошук не розрізняє варіанти апострофа.** Набране з клавіатури
  «здоров'я» знаходить «Громадське здоров’я» з U+2019 — інакше п'ять назв
  номенклатури не знаходились би взагалі.
- **Порожній вибір у проведенні означає «як у курсу»** і лягає в БД як
  NULL, а не порожній список. Прибрати спеціальності саме для проведення
  не можна: їх знімають у курсі.
- **Ліміту на кількість обраних немає, натомість рядок сертифіката сам
  зменшує шрифт:** до 60 символів — 12.5pt (сертифікат з однією
  спеціальністю не змінився), до 120 — 10.5pt, далі 9pt. Пороги міряні
  рендером під ширину метаблоку 166 мм, а не на око.
- **Виданий сертифікат — знімок рядка, а не посилання на довідник.**
  Перейменування спеціальності в адмінці не змінює вже виданих документів;
  знімок так само не залежить від мови сайту, на якій натиснули «видати», —
  він завжди українською.
- **Старий вільний текст із курсів не загубився:** міграція звірила його з
  довідником, а незбіги завела окремими рядками з `is_active=False`. Саме
  тому в довіднику можуть траплятись неактивні позиції, яких у номенклатурі
  немає, — курс, де вони стоять, показує свою назву далі.
- **Актуалізація номенклатури — робота в `/admin/specialties`, а не
  деплой:** додати рядок, перейменувати (окремо ru та en), змінити порядок,
  зняти активність. Інструмент розбору PDF потрібен лише для первинного
  наповнення.
- **Зайнятий рядок видалити не можна:** кнопка відмовить, назвавши число
  курсів і проведень, що його вживають, і запропонує зняти галку
  «Активна». Деактивований рядок зникає лише з вибору для нових заходів.
- **Права `specialties.*` роздаються вручну** в `/admin/access`. Сам
  `flask rbac sync` виконує воркфлоу деплою, але жодній ролі, крім
  `super_admin`, право не видається автоматично.
- **Відкат міграції односторонній для довгих переліків:** повертаючи
  колонку знімка в `String(500)`, він обрізає те, що не вміщається, — а
  шість назв у 500 символів уже не вміщаються.
