...
- **Партнерське API замінило ключ `cpd_points` парою `cpd_points_online` /
  ...
  заповнений.
- **Розетка на сертифікаті майже завжди десятибальна.** Файли лежать за
  конвенцією `{N}-points-BPR.webp`, а в каталозі є лише
  `10-points-BPR.webp`, тож усі інші номінали мовчки відкочуються на нього.
  Раніше це маскувалось помилкою в імені файлу (`9.00-points-BPR.webp`);
  тепер підстановка видима, і бракуючі розетки треба домалювати.
