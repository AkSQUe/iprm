...
**Гео:** проксі прокидає `X-Forwarded-For`, тож на відміну від GA локація
могла б визначатись коректно. Але в проєкті PostHog зараз стоїть
`anonymize_ips: true` -- IP відкидається при прийомі, і гео не буде, доки цей
прапорець не знімуть (див. TODO).
