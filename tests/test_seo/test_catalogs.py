...
# Лист «тестування відкрито» (13.09.2026): ru/en замовник перекладає сам.
# Коли переклад з'явиться, test_allowlist_entries_are_still_untranslated
# вимагатиме прибрати ці рядки звідси -- і так нагадає, що їх перекладено.
_QUIZ_INVITE_REASON = (
    'лист «тестування відкрито» (quiz_invite.html): переклад ru/en робить '
    'замовник окремо; до того лист у ru/en виходить українською.'
)

# Рядки, яким переклад свідомо не додається в цьому раунді:
# msgid -> причина. Виняток завжди іменований.
EMPTY_ALLOWLIST = {
    **{msgid: _QUIZ_INVITE_REASON for msgid in (
        'Тестування відкрито',
        'Тестування відкрито: %(title)s',
        'Складіть тест, щоб отримати сертифікат за %(title)s',
        'тестування за вашим заходом уже відкрите. Сертифікат із балами БПР '
        'видається лише тим, хто заповнив анкету для сертифіката і склав тест.',
        'Питань',
        'Потрібно правильних',
        'Спроб',
        'Скласти до',
        'Спершу заповніть анкету для сертифіката: без неї тест не відкриється. '
        'Це 2–3 хвилини, і анкета діє для всіх ваших курсів.',
        'Після анкети тест буде доступний за посиланням:',
    )},
