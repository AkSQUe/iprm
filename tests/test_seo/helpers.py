...
def looks_like_an_id(value):
    """Чи є голий рядок ПОСИЛАННЯМ на сутність, а не текстом.

    Голий рядок під ключем-посиланням має в JSON-LD дві різні долі. Під
    provider це скорочення для {'@id': ...} -- валідний IRI. Під author
    schema.org дозволяє звичайний Text: `'author': 'ІПРМ'` -- ім'я, а не
    ідентифікатор.

    Доти, доки будь-який рядок читався як @id, друга форма ламала сюїту:
    цілком коректний `'author': site_settings.t('company_name')` у
    blog/post.html давав два падіння з повідомленням "посилання
    @id='ІПРМ' немає серед вузлів організації". Це рівно той клас
    дефекту, заради усунення якого переписувався лічильник <h1>, --
    червона збірка на правильній розмітці.

    Тому рядок вважається посиланням лише тоді, коли він на посилання
    схожий: абсолютний URL, фрагмент (#org) або відносний шлях із
    кореня. Ім'я організації жодною з цих форм не є, а справжнє висяче
    посилання -- завжди одна з них.
    """
    if not isinstance(value, str) or not value:
        return False
    return is_absolute_url(value) or value.startswith(('#', '/'))
...
def _reference_ids(value):
    ...
    Рядок проходить через looks_like_an_id(): скорочена форма лишається
    покритою, а звичайний Text (author -- ім'я) більше не читається як
    розірване посилання.
    """
    if isinstance(value, dict):
        return [value['@id']] if '@id' in value else []
    if isinstance(value, str):
        return [value] if looks_like_an_id(value) else []
