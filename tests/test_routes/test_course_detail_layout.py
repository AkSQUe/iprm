...
далі одним шматком наше додаткове (історія -> сертифікат -> наступний крок
-> рефералка -> фінальний CTA).

Виняток з еталона -- блок найближчих дат: за пунктом 7 ТЗ він стоїть
одразу за повним описом, а не в підвалі, бо відвідувач мусить побачити
доступні варіанти ще під час читання опису.
...
SECTION_MARKERS = [
    ('про курс', 'id="about"'),
    # Дати йдуть одразу за повним описом (пункт 7 ТЗ): відвідувач мусить
    # побачити доступні варіанти ще під час читання, а не в підвалі.
