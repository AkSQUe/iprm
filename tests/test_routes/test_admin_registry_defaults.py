...
KNOWN_NON_NEUTRAL_DEFAULTS = {
    ...
    ('routes_registrations.py', 'view'):
        "view='list' -- дефолтний режим реєстру плаский, а не «За заходами»; "
        "рядок не ховається (обидва режими показують той самий зріз), тож "
        "жодне посилання на конкретну реєстрацію не опиняється поза "
        "видимим зрізом -- запис тут лише тому, що дефолт непорожній.",
}
