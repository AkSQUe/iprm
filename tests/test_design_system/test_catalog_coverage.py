...
# --- каталог не має підробляти компоненти -------------------------------
#
# Секція hero показувала заголовок як `font-size: clamp(2rem, 5vw, 3rem)`
# вагою 800, тоді як `.iprm-hero__title` має 24px і 700. Тобто вітрина
# малювала компонент удвічі більшим за нього самого -- значеннями,
# СКОПІЙОВАНИМИ з CSS, а не самим компонентом.
#
# Це найгірше місце для копії: у довідник дивляться саме щоб не робити
# копію. І поки inline-стиль задає вигляд, вітрина не ламається разом із
# компонентом -- тобто мовчить рівно тоді, коли має кричати.
#
# Риштування розкладки (`display`, `flex`, `margin`, `width`) сюди не
# входить: воно розставляє демо й нічого про компонент не стверджує.
#
# Храповик: 164 підробки лишились із попереднього стану вітрини. Тепер їх
# нуль, і baseline порожній -- тобто тест уже не "не дає побільшати", а
# тримає нуль: будь-який inline-стиль, що ЗАДАЄ ВИГЛЯД, валить його одразу.

MOCKUP_BASELINE = Path(__file__).parent / 'catalog_mockups_baseline.json'


def test_catalog_mockups_only_shrink():
    baseline = json.loads(MOCKUP_BASELINE.read_text(encoding='utf-8'))
    current = ds_audit.catalog_mockups(ROOT)
    grown = {name: (n, baseline.get(name, 0))
             for name, n in current.items() if n > baseline.get(name, 0)}
    assert not grown, (
        'у каталозі побільшало inline-стилів, що ЗАДАЮТЬ ВИГЛЯД: '
        + ', '.join('%s (%d проти %d)' % (n, v[0], v[1]) for n, v in grown.items())
        + '\nПоказуйте компонент ЖИВИМИ класами: інакше вітрина малює його '
          'копією значень, копія розходиться, і каталог починає брехати саме '
          'там, куди дивляться, щоб копії не робити. Риштування розкладки '
          '(display/flex/margin/width) дозволене -- воно нічого про компонент '
          'не стверджує.'
    )
