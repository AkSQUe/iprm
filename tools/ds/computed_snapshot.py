...
Відомий шум, який НЕ треба перевідкривати. `admin.blog_create` і
`admin.course_create` дають по 4 розбіжності на кнопках `.i18n-tabs__btn`:
між прогонами міняється, яка мовна вкладка активна, тож елемент
`.i18n-tabs__btn--active` то з'являється, то зникає. Два знімки НЕЗМІНЕНОГО
коду відтворюють це стабільно.

Розбіжність тут виглядає як `(відсутнє) є -> -`, тобто зникнення ЕЛЕМЕНТА, а
не зміна значення. Правка в CSS такого дати не може -- вона не додає й не
прибирає вузлів. Якщо ваш diff показує лише такі рядки на цих двох
сторінках, це шум; будь-що інше -- ні.

Мораль ширша за ці дві сторінки: перш ніж списати розбіжність на шум, зніміть
...
PROPS = [
    ...
    # object-fit / object-position -- ключові слова, а не розраховані
    # довжини, тож вони детерміновані й шуму не дають. Без них знімок не
    # бачив утиліти на кшталт "зображення заповнює рамку" взагалі:
    # width/height свідомо відсутні (див. вище), і від правила лишалось
    # нуль видимих інструментові властивостей.
    'object-fit', 'object-position',
    # resize -- теж КЛЮЧОВЕ СЛОВО, не розрахована довжина, тож детермінований
    # і шуму не дає. Без нього знімок не бачив зведення `resize: vertical`,
    # яке стояло в п'яти файлах: diff показував нуль там, де просто не міг
    # побачити змін, а це найгірший різновид зеленого.
    'resize',
    # transition-* СЮДИ КЛАСТИ МАРНО, і це не забудькуватість. Нижче цей
    # знімок сам вимикає переходи (transition: none !important в
    # add_init_script) -- інакше значення ловилось би посеред анімації й
    # гуляло б між прогонами. Тобто getComputedStyle тут ЗАВЖДИ віддасть
    # 0s / ease, хоч би що стояло в CSS.
    #
    # Наслідок, який треба знати: розходження кривої переходу цей
    # інструмент не бачить у принципі. Я вже раз додав ці властивості й
    # повірив, що тепер бачу, -- знімок мовчав не тому, що змін не було.
    # Такі правки перевіряються читанням CSS, не знімком.
...
# клас, ви робите його НЕЗІСТАВНИМ -- старий шлях зникає, новий
# з'являється, і значення до/після не порівнюються взагалі. Для правок у
# РОЗМІТЦІ цей знімок доказом не є.
...
_PATH_JS = """
  ...
  // Класи, які вішає СКРИПТ залежно від стану сторінки, а не розмітки.
  // У підпис вони йти не можуть: `scrolled` живе на <header>, тобто на
  // предку всього документа, і один тумблер на ньому переписує шлях
  // КОЖНОГО нащадка -- 180 «розбіжностей» там, де змінилось нічого.
  // Стилі самого елемента це не ховає: вони порівнюються як завжди, і
  // справжня правка в `.scrolled` покажеться зміною ЗНАЧЕННЯ.
  const RUNTIME_CLASSES = new Set(['scrolled', 'admin-th--sortable']);
  function sigOf(el) {
    const cls = Array.from(el.classList)
      .filter((c) => !RUNTIME_CLASSES.has(c)).sort().join('.');
