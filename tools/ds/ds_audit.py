...
def catalog_only_components(root=ROOT):
    ...
    **Межа, якої ця міра не перетне.** Ім'я класу, ЗІБРАНЕ в рантаймі
    конкатенацією рядків (`'prefix--' + variant`), невидиме для будь-якого
    статичного скану: літерального повного імені немає ніде -- ні в
    шаблонах, ні в скриптах. Такий клас виглядатиме мертвим, хоч він
    живий. Так було з `admin-toast--*` в `admin-status-select.js`, поки
    власний показ тоста не прибрали на користь спільного `iprmToast()`.

    Перш ніж видаляти щось за підказкою цієї міри, грепни КОРІНЬ імені
    (частину до `--`), а не повне ім'я.
