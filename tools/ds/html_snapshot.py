...
NORMALIZERS = [
    ...
    # ...і та сама марка, віддана скрипту через data-атрибут. /admin/access
    # кладе токен у data-csrf кореневого вузла матриці прав -- форму
    # `name="csrf_token"` вище цей випадок не ловить, і знімок показував
    # 1 розбіжність на РІВНОМУ місці, тобто був непридатний як інструмент.
    (re.compile(r'(data-csrf=")[^"]*'), r'\1SNAP'),
    ...
    # Прив'язка -- до ПІДПИСУ картки, а не до її класу: клас уже раз
    # перейменували (.backup-stats__value -> .admin-stat-card__value, коли
    # сторінка переїхала на спільні картки адмінки), нормалізатор мовчки
    # перестав збігатися, і вільне місце знову почало текти в знімок. Правило,
    # яке не спрацьовує, виглядає точно так само, як робоче.
    # `\s*` між тегами ОБОВ'ЯЗКОВИЙ (див. примітку про _BETWEEN_TAGS нижче):
    # у сирому HTML тут перенос рядка, і патерн без нього просто не збігається.
    (re.compile(r'(admin-stat-card__value">)[^<]*(</span>\s*<span class="admin-stat-card__label">\s*'
                + re.escape(LABEL) + ')'), r'\1SNAP-DISK\2'),
    # ...і ТОЙ САМИЙ розмір у банері-попередженні вище на тій же сторінці
    # (admin/backups.html: "Вільно лише 48.5 GB (5%)."). Картку нормалізували,
    # а банер ні -- і через це /admin/backups виринав як "змінена сторінка" у
    # звірці, зроблений за пів години до того: за сеанс знімків вільне місце
    # зсунулось на 1.6 GB, бо самі знімки його й витрачають.
    #
    # Режим noise цього не ловить: два його прогони йдуть підряд, і за ті
    # секунди число не встигає зсунутись. Тобто "шум нуль" тут означав лише
    # "за десять секунд не змінилось", а не "стабільне".
    (re.compile(r'(Вільно лише )[\d.,]+\s*[KMGT]?B \(\d+%\)'), r'\1SNAP-DISK'),
]
