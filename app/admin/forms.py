...
class SiteSettingsForm(FlaskForm):
    ...
    show_cookie_banner = BooleanField(
        'Показувати банер про cookie',
        description='Повідомлення внизу сторінки з кнопкою «Прийняти». Банер лише інформує — він не гейтить збір даних, тож вимкнення прибирає саме повідомлення, а не аналітику. Політика Cookie лишається доступною за посиланням у футері.',
    )
    ...
    transfer_after_days = IntegerField(
        'Пересадити після заходу можна протягом, діб',
        default=90,
        validators=[InputRequired(message='Вкажіть кількість діб (0 — пересадки після заходу немає)'),
                    NumberRange(min=0, max=1095)],
        description='Учасник не прийшов — стільки діб після заходу його ще можна перевести на інший. Ті, хто відвідав, склав тест або отримав сертифікат, не переносяться в будь-якому разі. 0 — після заходу не переносити.',
    )
