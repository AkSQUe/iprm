...
def bounded_token_arg(name, max_length):
    """Короткий рядковий параметр, що фізично влазить у колонку ('' -- ні).

    Текстовий відповідник `ranged_int_arg`: дозволене значення тут не
    список варіантів із випадної підказки (`_PLATFORM_OPTIONS` лишається
    зручністю для `<select>`), а те, що колонка взагалі здатна тримати --
    наприклад `MetaLead.platform`, `String(10)`, куди Meta пише значення
    payload без звірки зі своїм же переліком платформ. Задовге значення НЕ
    обрізаємо, як `text_arg` для пошуку: рядок, довший за колонку, свідомо
    не те, що там може лежати, і мовчазне обрізання дало б фільтр за
    значенням, якого в базі бути не може.
    """
