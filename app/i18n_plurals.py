...
PLURAL_FORMS = {
    ...
    # Примітка під переліком дат: "ще N групу вже набрано". Форми знахідні,
    # бо іменник тут -- додаток до безособового "набрано", а не підмет.
    'full_groups': {
        'uk': ('групу', 'групи', 'груп'),
        'ru': ('группу', 'группы', 'групп'),
        'en': ('group', 'groups'),
    },
    'years': {
        'uk': ('рік', 'роки', 'років'),
    ...
    # Тривалість заходу в рядку параметрів hero. Без цього ключа plural()
    # повертав сам ключ, і сторінка курсу писала "1 days".
    'days': {
        'uk': ('день', 'дні', 'днів'),
        'ru': ('день', 'дня', 'дней'),
        'en': ('day', 'days'),
    },
    # Розкривач під колонкою дат на сторінці курсу: "Ще N дат".
    'dates': {
        'uk': ('дата', 'дати', 'дат'),
        'ru': ('дата', 'даты', 'дат'),
        'en': ('date', 'dates'),
    },
    # Рядок середньої оцінки над картками відгуків курсу
    # (partials/_course_reviews.html) -- видимий двійник AggregateRating.
    'reviews': {
...
def plural(n, key, lang=None):
    ...
    # Дробові значення (4,5) мають власну форму -- родовий однини.
    # Слов'янське правило її не дає, бо працює із залишками цілого;
    # тому нецілі відсікаємо ДО int().
    try:
        number = abs(float(n))
