...
def export_events_to_xlsx():
    ...
    filename = f"events_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
...
def _extract_export_value(event, field_key):
    ...
    if field_key == 'price':
        return float(event.price) if event.price else 0
...
def import_events_from_xlsx(file_stream, user_id):
    ...
    if header_row is None:
        stats['errors'].append("Header row not found (looking for 'Nazva' column)")
        return stats
...
def _find_headers(ws):
    """Scan first 10 rows for the header row containing 'Nazva'.
    ...
    for row_idx in range(1, min(11, (ws.max_row or 1) + 1)):
        for col_idx in range(1, (ws.max_column or 1) + 1):
            ...
            if cell_value and str(cell_value).strip() == 'Nazva':
                col_map = {}
