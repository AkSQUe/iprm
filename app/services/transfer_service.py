...
def _registration_problems(reg):
    ...
    if reg.status == 'cancelled':
        ...
        problems.append('Участь уже зараховано — реєстрацію завершено')

    # Відмітка присутності ставить attended, status='completed' і нараховує
    # бали ЦПР. Перенести такий рядок означало б привезти на новий захід
    # реєстрацію, яка вже "відвідана", з балами за старий захід і правом на
    # сертифікат за курс, якого людина не проходила. Гвардія 7 ловить це
    # лише коли сертифікат уже видано, тобто запізно.
    #
    # Свідомо НЕ обнуляємо поля мовчки: перенесення відвіданої реєстрації --
    # майже напевно помилка адміна, і він має її побачити, а не отримати
    # тихе "виправлення" даних про присутність.
    if reg.attended:
