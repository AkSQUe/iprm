...
class SiteSettings(TranslatableMixin, TimestampMixin, db.Model):
    ...
    # Друге вікно, після заходу: скільки діб можна пересаджувати того, хто
    # НЕ прийшов. Окреме число, а не продовження попереднього, бо це
    # протилежна ситуація: там питання «чи не пізно перед заходом», тут --
    # «чи не задавно після нього».
    #
    # 0 -- пересадки після заходу немає взагалі (поведінка до 07.09.2026).
    # Учасник, який не з'явився, за §4.1 Політики не має права на
    # повернення, тож пересадка -- жест доброї волі, а не зобов'язання:
    # решту небезпечних станів (відвідав, сертифікат, тест, відкрита
    # заявка, незакрита доплата) тримають власні запобіжники.
    transfer_after_days = db.Column(
        db.Integer, default=90, nullable=False, server_default='90',
    )
