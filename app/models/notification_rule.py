...
# тригери мають admin-нотифікацію (email_confirm/reminder -- user-facing
# only). 'certificate' тут -- лише збій автовидачі після тесту; сам лист із
# сертифікатом учаснику йде тим самим тригером, але без правила.
EVENT_TYPES = [
    ...
    # для менеджера це один сигнал: реклама йде, а заявок немає.
    ('meta_lead', 'Збій приймання лідів Meta'),
    # Тест складено, а сертифікат не видався (немає даних БПР, збій PDF).
    # Учаснику при цьому обіцяно надіслати документ -- без листа адміну про
    # збій знав би лише лог сервера.
    ('certificate', 'Сертифікат не видано автоматично'),
]
