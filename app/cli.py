...
# Цифри довіри. Кожна мусить бути ПРАВДОЮ саме про цей курс, тому дві з
# чотирьох відрізняються від еталона:
#
# * «13 років досвіду тренера» -- це біографія Анастасії Серкутан із
#   еталонного лендінгу. Курс веде інша людина, і в базі у неї свій стаж,
#   тож рядок замінено на факт про формат, а не про особу;
# * кількість проведень береться з історії курсу (11 проведень у базі на
#   17.08.2026), а не з еталонних «6+».
#
# Якщо редакція захоче показати стаж тренера -- це поле `highlights` самого
# тренера, і заповнювати його треба реальними числами в /admin/trainers.
PLAZMOGEL_PROOF_STATS = [
    {'value': '11 груп', 'label': 'уже пройшли за програмою'},
    {'value': '100%', 'label': 'очна практика під контролем'},
    {'value': 'до 10', 'label': 'учасників у групі'},
...
@click.option('--with-tariffs', is_flag=True,
              help='Створити тарифи еталона (10000 / 15000 грн). Це гроші: '
                   'ціни з ІНШОГО проведення, тому за замовчуванням вимкнено.')
@with_appcontext
def seed_plazmogel(slug, force, dry_run, with_tariffs):
    ...
    try:
        ...
        # --- тарифи. Створення ЛИШЕ за явним --with-tariffs.
        #
        # Ціни 10 000 / 15 000 -- з еталонного лендінгу іншого проведення, а
        # цей курс продається за своєю ціною. Створити тариф означає змінити
        # те, що людина побачить і заплатить, тому автоматика тут мовчить,
        # поки їй прямо не скажуть.
        if with_tariffs:
            _seed_plazmogel_tariffs(log, course, force)
        else:
            log.skip('тарифи', 'ціни еталона -- з іншого проведення; '
                               'створюються лише з --with-tariffs')

        # --- цифри тренера НЕ заливаємо, і це не забутий шматок.
        #
        # В еталонному лендінгу вони належать Анастасії Серкутан («13 років у
        # косметології», «11 років у дерматології»). Курс, у який ми ллємо
        # контент, веде інша людина -- у неї в базі стоїть свій стаж. Записати
        # чужі числа під її іменем означало б опублікувати вигадані регалії
        # конкретної людини: тексти про метод переносяться між курсами вільно,
        # біографія -- ніколи.
        #
        # Заповнює редакція в /admin/trainers, з реальних даних тренера.
        click.echo('Тренер:')
        log.skip('цифри тренера',
                 'біографічні числа еталона належать іншому тренеру')
