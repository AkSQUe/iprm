...
def confirmation(registration_id):
    ...
    # `status != 'cancelled'`, а НЕ `status == 'pending'`. Підтвердження
    # участі не означає, що гроші надійшли: менеджер заходу погоджує людину
    # наперед, і саме в цьому стані реєстрація їде партнеру як «очікує
    # оплати». Вужча умова робила кнопку «Оплатити» в кабінеті глухим кутом --
    # там вона показується для БУДЬ-ЯКОЇ неоплаченої, а сторінка у відповідь
    # не давала ані LiqPay, ані рахунка, ані пояснення.
    #
    # `payment_status` лишається саме `unpaid`: `pending` тут ставить лише
    # LiqPay на `processing`/`wait_accept`, тобто гроші вже в дорозі, і
    # показати форму вдруге означало б запросити на друге списання. Для
    # нього нижче є своя гілка -- «оплата обробляється».
    needs_payment = (
