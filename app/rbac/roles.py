"""Операції над ролями: синк реєстру в БД, матриця, CRUD ролей, призначення.

Жодна функція не комітить: транзакцією керує маршрут або CLI. Кожна зміна
пише рядок в audit-логер. Запобіжники (розділ 6.5 спеки) кидають
``AccessError`` із текстом, придатним для flash.
"""
import logging
import re
...
from .access import invalidate_cache, is_super_admin

audit_logger = logging.getLogger('audit')

_SLUG_MAX = 50

#: Значення фільтра «Роль» у списку користувачів, що означає «без ролей».
#: Slug ролі з таким кодом заборонений, інакше фільтр не розрізнить їх.
NO_ROLE_FILTER = 'none'

#: Коди, які не можна брати для кастомної ролі: сентинел фільтра і всі
#: системні ролі реєстру, зокрема ще не створені (інакше sync побачить
#: «роль уже є» і мовчки не створить системну).
RESERVED_SLUGS = frozenset({NO_ROLE_FILTER}) | frozenset(registry.ROLES_BY_NAME)
...
class SystemActor:
    """Актор для CLI: діє як super_admin, але не є користувачем.

    ``id`` None, тож перевірки «сам собі» ніколи не спрацьовують, а
    ``assigned_by`` лишається порожнім (FK на users дозволяє NULL).
    """
    id = None
    email = 'cli'

    def __init__(self):
        self.roles = Role.query.filter_by(name=registry.SUPER_ADMIN).all()
...
def sync(dry_run=False):
    ...
    Наявних ролей і їхніх прав НЕ торкається: матриця головна. Право, якого
    більше немає в реєстрі, видаляється разом із видачами ролям. Не комітить.
    ``dry_run`` лише рахує, що змінилося б, і нічого не пише.
    """
    ...
    present_roles = {r.name for r in Role.query.with_entities(Role.name).all()}
    created = [spec.name for spec in registry.ROLES if spec.name not in present_roles]
    result = {'added': added, 'removed': removed, 'roles_created': created}
    if dry_run:
        return result
...
def _validate_slug(name):
    ...
    if name in RESERVED_SLUGS:
        raise AccessError(f'Код {name} зарезервований системою')
...
def update_role(role, display_name, description, color, sort_order, actor, name=None):
    """Системній ролі slug не міняється: форма рендерить поле readonly, а
    інший код у POST -- це або застаріла вкладка, або ручний запит; тихо
    ігноруємо замість помилки."""
    _validate_role_fields(display_name, color, sort_order)
